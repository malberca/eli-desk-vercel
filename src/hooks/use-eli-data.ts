...
export const tagSuggestions: Record<string, string[]> = {
  reclamo: ['plomería', 'electricidad', 'ascensor', 'limpieza', 'ruidos', 'humedad', 'filtraciones', 'pintura', 'portería'],
...
export async function softDeleteTicket(
  id: string,
  reason: string,
  deletedBy: string = 'dashboard-admin'
): Promise<{ success: boolean; error?: string }> {
  const { error } = await supabase
    .from('tickets')
    .update({
...
export function useChartData() {
  ...
  const fetchChartData = useCallback(async () => {
    try {
      ...
      const dateMap = new Map<string, { abiertos: number; cerrados: number; total: number }>()
      ...
      tickets?.forEach((ticket) => {
        ...
        if (!dateMap.has(createdDate)) {
          dateMap.set(createdDate, { abiertos: 0, cerrados: 0, total: 0 })
        }
