...
import { handleMcvClaimRequest } from "@/server/mcv-api/request-handler";
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    return await handleMcvClaimRequest(request, {
      environment: environment as BotSecurityEnvironment,
      keyRegistry: createRuntimeMcvKeyRegistry(),
      nonceStore: createSupabaseMcvReplayStore("ELI-N8N-MCV-CLAIM-V1"),
